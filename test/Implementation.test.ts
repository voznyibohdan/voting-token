...
describe('Implementation Contract', () => {
    ...
    async function deploy() {
        const [userAccount, fromAccount, toAccount, testAccount] =
    ...
    async function startVoting(contract: Implementation, user: any) {
        ...
        const expectedEndTime =
            Number(await contract.TIME_TO_VOTE()) +
    ...
    describe('Initial state', () => {
        it('Should set correct initial state', async () => {
            ...
            expect(await implContract.minTokenAmount()).to.equal(
                initialMinTokenAmount,
            );
            expect(await implContract.feePercentage()).to.equal(
                initialFeePercentage,
            );
        });
    });
    ...
    describe('BalanceOf', () => {
        it('Should return correct user balance', async () => {
            const { implContract, userAccount } = await loadFixture(deploy);
            await buy(implContract, userAccount, 10);
            expect(await implContract.balanceOf(userAccount.address)).to.equal(
                10,
            );
        });
    });
    ...
    describe('Transfer', () => {
        it('Should revert', async () => {
            const { implContract, fromAccount, toAccount } =
                await loadFixture(deploy);
            await expect(
                implContract.connect(fromAccount).transfer(zeroAddress, 1),
            ).to.revertedWith('Zero address');
            await startVoting(implContract, fromAccount);
            await expect(
                implContract.transfer(fromAccount.address, 1),
        ...
        it('Should change user balances, emit Transfer event', async () => {
            ...
            expect(await implContract.balanceOf(fromAccount.address)).to.equal(
                5,
            );
            expect(await implContract.balanceOf(toAccount.address)).to.equal(5);
            await expect(
                implContract.connect(fromAccount).transfer(toAccount, 5),
            )
    ...
    describe('Approve', () => {
        it('Should revert', async () => {
            ...
            await expect(implContract.approve(zeroAddress, 10)).to.revertedWith(
                'Zero address',
            );
        });
        ...
        it('Should set correct allowance and emit Approval event', async () => {
            ...
            expect(
                await implContract.allowances(
                    fromAccount.address,
                    toAccount.address,
                ),
            ).to.equal(50);
            await expect(
                implContract.connect(fromAccount).approve(toAccount.address, 1),
            )
    ...
    describe('TransferFrom', () => {
        it('Should revert', async () => {
            const { implContract, fromAccount, toAccount, testAccount } =
                await loadFixture(deploy);
            await expect(
                implContract.transferFrom(fromAccount.address, zeroAddress, 10),
            ).to.revertedWith('Zero address');
            await expect(
                implContract.transferFrom(
                    fromAccount.address,
                    toAccount.address,
                    10,
                ),
            ...
            await expect(
                implContract
                    .connect(testAccount)
                    .transferFrom(testAccount.address, toAccount.address, 1),
            ).to.revertedWith('Cant perform operation while voting is active');
        });
        ...
        it('Should change balances, change allowance, emit Transfer event', async () => {
            ...
            await implContract
                .connect(fromAccount)
                .transferFrom(fromAccount.address, toAccount.address, 1);
    ...
    describe('StartVoting', () => {
        it('Should revert', async () => {
            const { implContract, userAccount, fromAccount } =
                await loadFixture(deploy);
            await expect(implContract.startVoting(1)).to.revertedWith(
                'No tokens',
            );
            await buy(implContract, userAccount, 100);
            await expect(
                implContract.connect(fromAccount).startVoting(1),
        ...
        it('Should update votingId, voters, voting state, voting prices, leadingPrice, emit VotingStarted event', async () => {
            ...
            expect(await implContract.voters(userAccount.address)).to.equal(
                newVotingId,
            );
            expect(await implContract.isVotingInProgress()).to.equal(true);
            expect(await implContract.votingEndTime()).to.equal(
                expectedEndTime,
            );
    ...
    describe('Vote', () => {
        it('Should revert', async () => {
            ...
            await buy(implContract, userAccount, 100);
            await expect(
                implContract.connect(fromAccount).vote(1),
            ).to.revertedWith('Insufficient balance to execute this function');
            await buy(implContract, fromAccount, 100);
            await expect(implContract.vote(1)).to.revertedWith(
        ...
        it('Should update voters, set voting price, emit Voted event', async () => {
            ...
            expect(await implContract.voters(fromAccount.address)).to.equal(
                votingId,
            );
    ...
    describe('EndVoting', () => {
        it('Should revert', async () => {
            const { implContract, userAccount } = await loadFixture(deploy);
            await expect(implContract.endVoting()).to.revertedWith(
        ...
        it('Should set new tokenPrice, set leadingPrice to zero, VotingEnded', async () => {
            const { implContract, userAccount } = await loadFixture(deploy);
            const { votingPrice } = await startVoting(
                implContract,
                userAccount,
            );
            await ethers.provider.send('evm_increaseTime', [3600]);
            await expect(implContract.endVoting()).emit(
                implContract,
                'VotingEnded',
            );
    ...
    describe('Buy', () => {
        it('Should revert', async () => {
            const { implContract, userAccount } = await loadFixture(deploy);
            await expect(
                implContract.connect(userAccount).buy(10, { value: 9 }),
        ...
        it('Should increase the users balance, total amount, etherPool, feePool, emit transfer event', async () => {
            const { implContract, userAccount } = await loadFixture(deploy);
            await expect(
                implContract.connect(userAccount).buy(10, { value: 10 }),
            )
                .emit(implContract, 'Transfer')
                .withArgs(zeroAddress, userAccount.address, 10);

            expect(await implContract.totalSupply()).to.equal(10);
            expect(await implContract.balances(userAccount.address)).to.equal(
                10,
            );
    ...
    describe('Sell', () => {
        it('Should revert', async () => {
            const { implContract, userAccount } = await loadFixture(deploy);
            await expect(implContract.sell(10)).to.revertedWith(
                'Insufficient tokens',
            );
            await startVoting(implContract, userAccount);
            await expect(implContract.sell(1)).to.revertedWith(
    ...
    describe('BurnFee', () => {
        it('Should revert', async () => {
            const { implContract } = await loadFixture(deploy);
            await expect(implContract.burnFee()).to.revertedWith(
                'A week has not yet passed since the last burning',
            );
        });
